...
import { collection, addDoc, onSnapshot, updateDoc, doc, deleteDoc, query } from "firebase/firestore";
...
const TeacherDashBoard = ({ isDark }) => {
  ...
  useEffect(() => {
    const unsubscribeAuth = onAuthStateChanged(auth, (currentUser) => {
      if (currentUser) {
        setUser(currentUser);
        // 1. Classes
        const unsubClasses = onSnapshot(query(collection(db, "timetable")), (snapshot) => {
  ...
  const handleAddClass = async (e) => {
    ...
    setProcessing(true);
    try {
      await addDoc(collection(db, "timetable"), { ...newClass, teacherId: user.uid, createdAt: new Date() });
